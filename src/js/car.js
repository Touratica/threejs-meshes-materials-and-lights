class Car extends Component{
	...
	createBody(geometry, radius, x, y, z) {
		...
		let phongMat = new THREE.Mesh(geometry,
			new THREE.MeshPhongMaterial({color: bodyworkColor, specular: 0x424849, side: THREE.DoubleSide}));
		let lambertMat = new THREE.Mesh(geometry,
			new THREE.MeshLambertMaterial({color: bodyworkColor, side: THREE.DoubleSide}));
		let basicMat = new THREE.Mesh(geometry,
			new THREE.MeshBasicMaterial({color: bodyworkColor,side: THREE.DoubleSide}));
	...
	createWindow(x, y, z, radius) {	
		...
		let basicMat = new THREE.Mesh(geometry,
			new THREE.MeshBasicMaterial({color: windowColor, side: THREE.DoubleSide}));
